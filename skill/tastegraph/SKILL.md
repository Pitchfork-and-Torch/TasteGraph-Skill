...
2. Evidence summaries stay careful; sample graphs in this repo stay synthetic.
...
4. Skill sample graphs stay **synthetic**. Product chrome (decision wheel) may show real **public** posts with credit.

## Public wheel examples (standing)

When adding or refreshing stills on the interactive decision wheel (`site/wheel-app`):

1. If the still is from a **public** art, design, type, or UI post, set **both** `credit` (`@handle`) and `href` (canonical post URL, usually `https://x.com/.../status/...`).
2. Source links are **standard fare** - not optional polish. Caption-only public-art examples are incomplete.
3. Synthetic or operator-owned stills may omit `href`; mark credit as synthetic.
4. Gallery UI renders "Source @handle" from those fields (`src/main.js`). Keep that path working.
5. **Update log:** every material public wheel or site ship appends a newest-first entry to `site/public/updates/index.html` and `site/public/updates/log.json`.
...
- Shipping public wheel examples from art posts without a source `href` when the URL is known
