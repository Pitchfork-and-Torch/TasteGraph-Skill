...
async function loadContributingSources() {
  ...
  const url = `${base}contributing-sources.json?v=${GRAPH_PUBLIC_VERSION || "1"}`;
  try {
...
function setPanel(slice) {
  ...
  slice.examples.forEach((ex) => {
    ...
    const src = base + String(ex.src || "").replace(/^\//, "");
    img.src = src;
    img.alt = ex.caption || "";
    img.loading = "lazy";
    img.decoding = "async";
...
async function loadIngestStills() {
  const base = import.meta.env.BASE_URL || "/";
  const url = `${base}ingest-stills.json?v=${GRAPH_PUBLIC_VERSION || "1"}`;
  try {
    const res = await fetch(url, { cache: "no-cache" });
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    ...
    // Fallback: featured stills from slices only
    ingestCatalog = {
      count: 0,
    ...
    ingestCatalog.count = ingestCatalog.items.length;
  }
  buildGridFilters();
  renderIngestGrid();
}
...
function renderIngestGrid() {
  ...
  if (countEl) {
    ...
    if (gridFilter === "all" && total) countEl.textContent = `${total} stills ingested`;
  }
  grid.innerHTML = "";
  if (!items.length) {
  ...
  items.forEach((it) => {
    const fig = document.createElement("figure");
    fig.className = "ingest-card";
    fig.tabIndex = 0;
    const img = document.createElement("img");
    const src = base + String(it.src || "").replace(/^\//, "");
    img.src = src;
